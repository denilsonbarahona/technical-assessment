...
const getMinutes = (milliseconds) => milliseconds / (1000 * 60);
...
const getTimeFormat = (date) => {
  /**current date */
  try {
    if (date === null) throw new Error("Invalid time value");
    const Now = new Date();
    ...
    switch (true) {
      ...
      case getSeconds(milliseconds) < 60:
        return intl.format(
          Math.floor(-1 * getSeconds(milliseconds)),
          "seconds"
        );
      /**check if the time passed are minutes */
      case getMinutes(milliseconds) < 60:
        return intl.format(
          Math.floor(-1 * getMinutes(milliseconds)),
          "minutes"
        );
